...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    valid_paragraph = [paragraph for paragraph in paragraphs if select(paragraph)]
...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def check_words_in_s(s):
        s_inwords = split(lower(remove_punctuation(s)))

        for word in s_inwords:
            if word in subject:
                return True
        return False   
    
    return check_words_in_s
    # END PROBLEM 2
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        diff_word = [diff_function(typed_word, word, limit) for word in word_list]
        min_diff = min(diff_word, key = abs)
        close_word = word_list[diff_word.index(min_diff)]
...
def minimum_mewtations(typed, source, limit):
    ...
    3
    """
    if len(typed) == 0 or len(source) == 0 or limit < 0: # Base cases should go here, you may add more base cases as needed.
        # BEGIN
        "*** YOUR CODE HERE ***"
        return len(typed) + len(source)
        # END
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    times = [[timestamps_per_player[i][j+1] - timestamps_per_player[i][j] for j in range(len(timestamps_per_player[i])-1)] 
...
def fastest_words(match):
    ...
    for i in player_indices:
        ...
        for j in word_indices:
            times_i = [time(match, k, j) for k in player_indices]
            if times_i.index(min(times_i)) == i:
                player_i_fast.append(get_word(match, j))    
        fast.append(player_i_fast)
